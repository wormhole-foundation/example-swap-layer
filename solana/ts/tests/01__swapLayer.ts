...
describe("swap-layer", () => {
  ...
  const foreignEndpointAddress = Array.from(
    Buffer.alloc(
      32,
      "000000000000000000000000603541d1Cf7178C407aA7369b67CB7e0274952e2",
      "hex"
    )
  );
  ...
  // Program SDKs
  const swapLayer = new SwapLayerProgram(
    connection,
    localnet(),
    USDC_MINT_ADDRESS
  );
  ...
  describe("Admin", () => {
    describe("Initialize", () => {
      ...
      after("Setup Lookup Table", async function () {
        // Create.
        const [createIx, lookupTable] = await connection
          .getSlot("finalized")
          .then((slot) =>
        ...
        const extendIx = AddressLookupTableProgram.extendLookupTable({
          ...
          authority: payer.publicKey,
          lookupTable,
          addresses: Object.values(usdcCommonAccounts).filter(
            (key) => key !== undefined
          ),
        });
  ...
  describe("Business Logic", function () {
    ...
    describe("USDC Transfers", function () {
      ...
      it("Redeem USDC", async function () {
        ...
        const preparedFill = tokenRouter.preparedFillAddress(
          vaaAccount.digest()
        );
...
async function craftCctpTokenBurnMessage(
  ...
  const burnMessage = new CctpTokenBurnMessage(
    ...
    0,
    Array.from(
      wormholeSdk.tryNativeToUint8Array(ETHEREUM_USDC_ADDRESS, "ethereum")
    ), // sourceTokenAddress
    encodedMintRecipient,
    amount,
    burnSource
  ...
  const cctpAttestation = new CircleAttester().createAttestation(
    encodedCctpMessage
  );
